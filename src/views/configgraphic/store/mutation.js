...
export default {
  ...
  select (state, payload) {
    ...
    if (payload.uuid === -1) {
      ...
      const widget = state.widgets.find(w => w.uuid === payload.uuid)
  ...
  addWidget (state, { data: data = null, item }) {
    const def = { top: state.top, uuid: generate('1234567890abcdef', 10) }
    const setting = JSON.parse(JSON.stringify(item.setting))
  ...
  updateData (state, { uuid, key, value }) {
    const widget = state.widgets.find(w => w.uuid === uuid)
    widget[key] = value
  }
}
